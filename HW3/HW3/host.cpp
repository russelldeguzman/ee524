...
const char kernel_func_name[] = "helloparallelworld";
...
int main(int argc, char** argv)
{
	...
	for (cl_uint i = 0; i < num_platforms; i++) {
		...
		for (cl_int j = 0; j < num_devices; j++)
		{
			...
			for (cl_int k = 0; k < dev_max_item_dimensions; k++)
			{
	...
	file_contents = read_source("device.cl", &file_size);
	...
	kernel = clCreateKernel(program, kernel_func_name, &clStatus);
	if (clStatus != CL_SUCCESS)
	{
